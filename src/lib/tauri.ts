...
import type { Setting, Reference, BibliographyInfo } from '$lib/types';
...
export async function searchByField(
  ...
  return await invoke<Reference[]>('search_by_field', { references, query, field });
}
...
// Check for updates
export async function checkUpdate(): Promise<{ available: boolean; version?: string; notes?: string }> {
