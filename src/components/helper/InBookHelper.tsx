...
interface InBookHelperProps {
  entry: Reference;
}

function InBookHelper({ entry }: InBookHelperProps) {
    ...
    <div className="w-full">
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-2">
          <div className="badge badge-accent badge-soft badge-sm font-bold">
            InBook
          </div>
          {entry.title
            ? (
              ...
              ? (
                <>
                  {entry.author.slice(0, 3).map((author, idx) => (
                    <span
                      key={idx}
                      className="badge badge-ghost badge-xs hover:badge-accent transition-colors cursor-default"
                    >
                      {author}
                    </span>
                  ))}
                  <span className="badge badge-ghost badge-xs hover:badge-accent transition-colors cursor-default">
                    et al.
                  </span>
                </>
              )
