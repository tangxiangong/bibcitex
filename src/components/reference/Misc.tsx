...
function Misc({ entry }: MiscProps) {
  ...
  const arxiv = (() => {
    ...
    const primaryClass = entry.arxiv_primary_class;
  ...
  return (
    <div className="card-modern card-shine group hover:-translate-y-1 transition-all duration-300 m-4 border-l-4 border-l-error">
          ...
          {entry.author && entry.author.length > 0
            ? (
              entry.author.map((author, idx) => (
                <span
                  key={idx}
                  className="badge badge-ghost hover:badge-error transition-colors cursor-default bg-base-200/50"
                >
                  {author}
                </span>
              ))
