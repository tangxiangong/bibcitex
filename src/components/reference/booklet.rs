...
pub fn Booklet(entry: Reference) -> Element {
    ...
    rsx! {
        div { class: "card-modern card-shine group hover:-translate-y-1 transition-all duration-300 m-4 border-l-4 border-l-cyan-500",
            div { class: "card-body p-5",
                ...
                div { class: "mt-4 flex flex-wrap items-center gap-4 text-sm text-base-content/70 border-t border-base-content/5 pt-3",
                    ...
                    if let Some(file) = entry.file {
                        button {
                            class: "btn btn-xs btn-info btn-soft gap-1",
                            onclick: move |_| {
...
#[component]
pub fn BookletDrawer(entry: Reference) -> Element {
    let key = &entry.cite_key;
