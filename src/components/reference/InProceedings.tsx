...
function InProceedings({ entry }: InProceedingsProps) {
  ...
  return (
    <div className="card-modern card-shine group hover:-translate-y-1 transition-all duration-300 m-4 border-l-4 border-l-purple-500">
      <div className="card-body p-5">
        {/* Header: Type + Title + Actions */}
        <div className="flex justify-between items-start gap-4">
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-2">
              <span className="badge badge-purple badge-soft badge-sm font-bold">
                InProceedings
              </span>
          ...
          {entry.author && entry.author.length > 0
            ? (
              entry.author.map((author, idx) => (
                <span
                  key={idx}
                  className="badge badge-ghost hover:badge-purple transition-colors cursor-default bg-base-200/50"
                >
                  {author}
                </span>
              ))
        ...
        <div className="mt-4 flex flex-wrap items-center gap-4 text-sm text-base-content/70 border-t border-base-content/5 pt-3">
          ...
          {entry.doi && (
            <button
              type="button"
              className="btn btn-xs btn-ghost gap-1 hover:text-purple"
          ...
          {entry.file && (
            <button
              type="button"
              className="btn btn-xs btn-purple btn-soft gap-1"
