...
function Book({ entry }: BookProps) {
  ...
  return (
    <div className="card-modern card-shine group hover:-translate-y-1 transition-all duration-300 m-4 border-l-4 border-l-emerald-500">
      <div className="card-body p-5">
          ...
          {entry.author && entry.author.length > 0
            ? (
              entry.author.map((author, idx) => (
                <span
                  key={idx}
                  className="badge badge-ghost hover:badge-success transition-colors cursor-default bg-base-200/50"
                >
                  {author}
                </span>
              ))
        ...
        <div className="mt-4 flex flex-wrap items-center gap-4 text-sm text-base-content/70 border-t border-base-content/5 pt-3">
          ...
          {entry.doi && (
            <button
              type="button"
              className="btn btn-xs btn-ghost gap-1 hover:text-success"
          ...
          {entry.file && (
            <button
              type="button"
              className="btn btn-xs btn-success btn-soft gap-1"
